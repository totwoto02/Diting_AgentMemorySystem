...
class TestMCPServerCoverageOptimization:
    ...
    @pytest.mark.asyncio
    async def test_line95_diting_read_missing_path(self, server):
        """测试第 95 行 - diting_read 缺少 path 参数"""
        arguments = {}  # 缺少 path
        result = await server._diting_read(arguments)
        assert "缺少 path 参数" in result[0].text

    @pytest.mark.asyncio
    async def test_line105_diting_write_missing_params(self, server):
        """测试第 105 行 - diting_write 缺少必需参数"""
        # 缺少 content
        arguments = {"path": "/test", "type": "RULE"}
        result = await server._diting_write(arguments)
    ...
    @pytest.mark.asyncio
    async def test_line170_171_diting_search_missing_query(self, server):
        """测试第 170-171 行 - diting_search 缺少 query 参数"""
        arguments = {}  # 缺少 query
        result = await server._diting_search(arguments)
    ...
    async def test_line192_call_tool_generic_exception(self, server):
        ...
        # 模拟一个未处理的异常
        with patch.object(server, '_diting_read', side_effect=RuntimeError("未预期的错误")):
            result = await server.call_tool("diting_read", {"path": "/test"})
            assert "系统错误" in result[0].text

    @pytest.mark.asyncio
    async def test_diting_read_not_found(self, server):
        """测试 diting_read 路径不存在的情况"""
        # 先写入再删除，确保路径不存在
        server.mft.create("/test/to_delete", "RULE", "test content")
        server.mft.delete("/test/to_delete")
        
        with pytest.raises(MFTNotFoundError):
            await server._diting_read({"path": "/test/to_delete"})

    @pytest.mark.asyncio
    async def test_diting_write_create_and_update(self, server):
        """测试 diting_write 创建和更新流程"""
        # 创建新记录
    ...
    @pytest.mark.asyncio
    async def test_diting_search_no_results(self, server):
        """测试 diting_search 无结果的情况"""
        arguments = {"query": "不存在的关键词"}
        result = await server._diting_search(arguments)
        assert "未找到匹配" in result[0].text

    @pytest.mark.asyncio
    async def test_diting_search_with_results(self, server):
        """测试 diting_search 有结果的情况"""
        # 先创建一些测试数据
    ...
    @pytest.mark.asyncio
    async def test_call_tool_diting_read(self, server):
        """测试 call_tool 调用 diting_read"""
        # 先创建数据
        server.mft.create("/test/read_test", "RULE", "test content")
        
        result = await server.call_tool("diting_read", {"path": "/test/read_test"})
        assert "路径：" in result[0].text
        assert "类型：" in result[0].text
        assert "内容：" in result[0].text

    @pytest.mark.asyncio
    async def test_call_tool_diting_write(self, server):
        """测试 call_tool 调用 diting_write"""
    ...
    @pytest.mark.asyncio
    async def test_call_tool_diting_search(self, server):
        """测试 call_tool 调用 diting_search"""
        server.mft.create("/test/search_test", "FACT", "这是搜索测试")
        
        result = await server.call_tool("diting_search", {"query": "搜索"})
        assert "找到" in result[0].text or "未找到" in result[0].text

    @pytest.mark.asyncio
    async def test_call_tool_diting_error(self, server):
        """测试 call_tool 错误处理"""
        # 测试 MFTNotFoundError
    ...
    async def test_list_tools_registered(self, server):
        ...
        assert "diting_read" in tool_names
        assert "diting_write" in tool_names
        assert "diting_search" in tool_names
