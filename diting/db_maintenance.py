...
class DatabaseMaintenance:
    ...
    def preview_archived_cleanup(self, retention_days: int = 365) -> dict:
        """
        T049: 预览将要清理的归档数据

        Args:
            retention_days: 保留天数，默认 365 天
        ...
        try:
            archived_tables = [
                "archived_audit_log",
                "archived_temperature_log",
                "archived_entropy_log",
                "archived_system_log",
                "archived_heat_log",
                "archived_kg_aliases",
            ]
            ...
            for table_name in archived_tables:
                ...
                time_col = self._get_time_column(conn, table_name)
                if time_col is None:
                    results[table_name] = 0
                    continue

                count = conn.execute(
                    f"SELECT COUNT(*) FROM [{table_name}] WHERE [{time_col}] < ?",
                    (cutoff,),
                ).fetchone()[0]
            ...
            return {
                "operation": "preview_archived_cleanup",
                "retention_days": retention_days,
                "cutoff_date": cutoff,
                "tables": results,
                "total_to_cleanup": total,
            }
        finally:
            conn.close()

    def cleanup_archived_data(self, retention_days: int = 365, dry_run: bool = False) -> dict:
        """
        T049: 清理归档数据（永久删除）

        安全机制：
        - 检测 OPENCLAW_AGENT 环境变量，阻止 OpenClaw 自动执行
        - 双重确认：输入 'DELETE' 和 'archived_memory'
        ...
        # 安全机制 1: 检测 OpenClaw 环境
        if os.environ.get("OPENCLAW_AGENT"):
            raise RuntimeError(
                "Cleanup blocked: OPENCLAW_AGENT environment variable detected. "
                "This operation must be triggered by a human user."
            )

        preview = self.preview_archived_cleanup(retention_days)

        if dry_run:
            return {
                "operation": "cleanup_archived_data",
                "dry_run": True,
                **preview,
            }

        if preview["total_to_cleanup"] == 0:
            return {
                "operation": "cleanup_archived_data",
                "dry_run": False,
                "deleted": 0,
                "message": "No archived data to cleanup",
            }
        ...
        try:
            ...
            for table_name, count in preview["tables"].items():
                if count == 0:
                    continue
                ...
                deleted_details[table_name] = cursor.rowcount
    ...
    def _get_time_column(self, conn, table_name: str) -> str:
        """获取表的时间列名"""
        cursor = conn.execute(f"PRAGMA table_info([{table_name}])")
        columns = {row[1] for row in cursor.fetchall()}

        # 优先级：timestamp > changed_at > created_at
