...
class MonitorDashboard:
    ...
    def _init_schema(self):
        ...
        # 创建索引
        self.db.execute("CREATE INDEX IF NOT EXISTS idx_metric_time ON monitor_metrics(metric_name, timestamp)")
    ...
    def get_system_status(self) -> Dict:
        ...
        # 数据库统计
        cursor = self.db.execute("SELECT COUNT(*) FROM monitor_metrics WHERE timestamp > datetime('now', '-1 hour')")
    ...
    def check_alerts(self) -> List[Alert]:
        ...
        # 磁盘使用率
        if system_status['system']['disk_percent'] / 100 > self.alert_rules['disk_usage']['threshold']:
            alert = Alert(
        ...
        # 内存使用率
        if system_status['system']['memory_percent'] / 100 > self.alert_rules['memory_usage']['threshold']:
            alert = Alert(
