...
class MultimodalMemoryManager:
    ...
    def __init__(self, db_path: str, storage_dir: str, config: Dict = None):
        ...
        self.enable_smart_trigger = self.config.get(
        ...
        # 智能触发阈值
        self.ai_trigger_filesize = self.config.get(
            'AI_TRIGGER_FILESIZE', 1024 * 1024)  # 1MB
        self.ai_trigger_max_monthly = self.config.get(
            'AI_TRIGGER_MAX_MONTHLY', 100)
    ...
    def _ensure_storage_dirs(self):
        ...
        # 按日期组织目录结构
        base_dirs = [
            self.storage_dir / 'images' / datetime.now().strftime('%Y') /
            datetime.now().strftime('%m'),
            self.storage_dir / 'audio' / datetime.now().strftime('%Y') /
            datetime.now().strftime('%m'),
    ...
    def store_image(self, image_data: bytes, memory_path: str,
        ...
        """, (
            ...
            ai_summary, json.dumps(ai_keywords), json.dumps(
                ai_entities), ai_confidence,
            'warm', 50,
        ...
        search_vector = self._build_search_vector(
            ai_summary, ai_keywords, ai_entities)
        self.db.execute("""
    ...
    def store_audio(self, audio_data: bytes, memory_path: str,
        ...
        """, (
            ...
            ai_summary, json.dumps(
                ai_keywords), raw_transcript, duration_seconds,
            'warm', 50,
            datetime.now().isoformat(), datetime.now().isoformat()
        ))
        ...
        # 7. 构建搜索向量
        search_vector = self._build_search_vector(
            ai_summary, ai_keywords, transcript=raw_transcript)
        self.db.execute("""
            UPDATE multimodal_slices
            SET search_vector = ?
            WHERE slice_id = ?
    ...
    def _save_file(self, file_data: bytes, file_type: str,
        ...
        extension = original_filename.split(
            '.')[-1] if original_filename else ('jpg' if file_type == 'image' else 'ogg')
    ...
    def search(self, query: str,
               include_multimodal: bool = True) -> List[Dict]:
        """搜索记忆（包含多模态）"""
        results = []
