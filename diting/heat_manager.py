...
class HeatManager:
    ...
    def __init__(self, db_path: str, config: Dict = None):
        ...
        # 温度配置
        self.time_decay_rate = self.config.get('TIME_DECAY_RATE', 0.1)  # 每天衰减 0.1 分
        self.round_decay_rate = self.config.get('ROUND_DECAY_RATE', 5)  # 每轮衰减 5 分
        self.user_heat_bonus = self.config.get('USER_HEAT_BONUS', 30)   # 用户主动升温 +30 分
    ...
    def _ensure_schema(self):
        ...
        if 'heat_score' not in columns:
            self.db.execute("ALTER TABLE multimodal_slices ADD COLUMN heat_score INTEGER DEFAULT 50")
        if 'last_heated_at' not in columns:
            self.db.execute("ALTER TABLE multimodal_slices ADD COLUMN last_heated_at TIMESTAMP")
        if 'freeze_reason' not in columns:
            self.db.execute("ALTER TABLE multimodal_slices ADD COLUMN freeze_reason TEXT")
        if 'freeze_by' not in columns:
            self.db.execute("ALTER TABLE multimodal_slices ADD COLUMN freeze_by TEXT")
        if 'freeze_at' not in columns:
            self.db.execute("ALTER TABLE multimodal_slices ADD COLUMN freeze_at TIMESTAMP")
        if 'last_mentioned_round' not in columns:
            self.db.execute("ALTER TABLE multimodal_slices ADD COLUMN last_mentioned_round INTEGER")
        if 'iteration_status' not in columns:
            self.db.execute("ALTER TABLE multimodal_slices ADD COLUMN iteration_status TEXT DEFAULT 'active'")
        ...
        """)
        self.db.execute("CREATE INDEX IF NOT EXISTS idx_temp_log_slice ON heat_log(slice_id)")
        self.db.commit()
